########################################################################
#
# Statistical models TBI FC data
...
fit_model <- function( df, set, outdir, percentage = FALSE )
{
	...
	m <- glmmTMB::glmmTMB( FC ~ Group + Timepoint + ( 1 | Rat ), data = df, family = gaussian() )
	lm  <- lme4::lmer( FC ~ Group + Timepoint + Timepoint*Group + ( 1 | Rat ), data = df)

	# post-hoc comparisons Group and Day
	g_grp <- multcomp::glht( m, linfct = mcp( Group = "Tukey" ) )
	g_day <- multcomp::glht( m, linfct = mcp( Timepoint = "Tukey" ) )
	g_int <- lsmeans( lm, pairwise~Timepoint*Group, adjust = "tukey" )
...
rois <- c( "CPu", "Hippocampus", "M1", "S1FL", "CPu-M1", "CPu-S1FL", "M1-S1FL") 
...
for( roi in rois )
{
	...
	# read data
	head( df <- read.csv( paste0( indir, '/FC_data.csv' ), sep=";", dec="," ) )
      
      # reorder levels timepoints
      df$Timepoint <- factor(df$Timepoint, levels=c("pre", "1h", "24h", "1w", "1m", "3m-4m"))
      
      # make selection based on roi and metric
      df.select <- df[df$lable==roi,]
      
      # metric definition
      metric <- paste(df[df$lable==roi,]$Type[[1]])
