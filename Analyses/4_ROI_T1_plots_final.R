...
## Define outdir
outdir <- "/home1/michel/projects/TBI/analyses/output_T1_final/"
dir.create(outdir)
...
# Construct factors
df.merged$Class <- factor(df.merged$Class, levels=c("Control", "Mild", "Moderate") )
...
dir.create(paste(outdir, "smooth/", sep=""))
...
plot <- ggplot( data = df.merged[df.merged$Metric=="t1" & df.merged$variable!="cp",], aes( x = Timepoint, y = value, 			group = Class, fill = Class, color = Class ) ) +
...
ggsave( file = outfile, plot = plot, dpi = 500, width = 12, height = 6 ) 
