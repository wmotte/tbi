...
############################################################################################################
## Institution: University Medical Center Utrecht ## Script: Determine ROI T1 values of all rats ##########
########################################################################################################
...
setwd("/home1/michel/projects/TBI/processed/T1/")
wd <- getwd()

## Define outdir
outdir <- "/home1/michel/projects/TBI/analyses/output_T1_11-1-2020/"
dir.create(outdir)
...
#1 Read in table with ROI labels
...
## Calculate mean +/- SD T1 for all ROIs for all timepoints for controls and TBI animals
...
plot <- ggplot( data = df.merged1[df.merged1$Metric=="t1",], aes( x = Timepoint, y = value, group = Class, fill = Class, color = Class ) ) +
  	geom_smooth() + scale_y_continuous(labels=scaleFUN, breaks=scales::pretty_breaks(n=3), limits=c(0,40)) +
	...
	#ggtitle( paste(as.character("T1 over time"), "\n", sep="") ) + 
...
outfile <- paste( outdir, "/T1-combi-smooth.png", sep = "" )
ggsave( file = outfile, plot = plot, dpi = 500, width = 10, height = 4 ) 

outfile <- paste( outdir, "/T1-combi-smooth.pdf", sep = "" )
ggsave( file = outfile, plot = plot, dpi = 500, width = 10, height = 4 ) 

## plot second
scaleFUN <- function(x) sprintf("%.1f", x)

plot <- ggplot( data = df.merged2[df.merged2$Metric=="t1",], aes( x = Timepoint, y = value, group = Class, fill = Class, color = Class ) ) +
  	geom_smooth() + scale_y_continuous(labels=scaleFUN, breaks=scales::pretty_breaks(n=3), limits=c(0,40)) +
	facet_wrap(~roi, scales="free_y", ncol=3) + 
  	xlab( "" ) +
  	ylab( paste("", sep="") ) + 
...
outfile <- paste( outdir, "/T1-combi-smooth2.png", sep = "" )
ggsave( file = outfile, plot = plot, dpi = 500, width = 10, height = 4 ) 

outfile <- paste( outdir, "/T1-combi-smooth2.pdf", sep = "" )
ggsave( file = outfile, plot = plot, dpi = 500, width = 10, height = 4 ) 


## To perform some linear mixed model analyses with glmmTMB (laptop), write CSV file
df.select <- df.merged[df.merged$Metric=="t1" & df.merged$variable!="cp",]

write.table(df.select, "/home1/michel/projects/TBI/analyses/data_final/t1_data.csv", sep=";", dec=",", row.names=FALSE)
