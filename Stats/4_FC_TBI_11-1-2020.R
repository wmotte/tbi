...
## Define outdir
outdir <- "/home1/michel/projects/TBI/analyses/output_FC_11-1-2020/"
dir.create(outdir)
...
df.all$Groep[df.all$Group=="Control"] <- "Sham"
df.all$Groep[df.all$Group=="TBI" & df.all$Condition=="Mild"] <- "Mild TBI"
df.all$Groep[df.all$Group=="TBI" & df.all$Condition=="Moderate"] <- "Moderate TBI"
df.all$Group <- NULL
df.all$time <- NULL
...
ggsave( file = outfile, plot = plot, dpi = 500, width = 8, height = 8 ) 
