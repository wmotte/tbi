...
# read data
head( df <- read.csv( paste0( indir, '/logistic_data_mild.csv' ), sep=";", dec="," ) )
df$response <- NULL
df$response[df$Class=="Mild"] <- 1
df$response[df$Class=="Control"] <- 0
...
write.table(as.data.frame(aovmodels1h), paste(getwd(),"/",outdir,"/logr_models_mild_sham.csv", sep=""), sep=";", dec=",")
...
write.table(Rm.total, paste(getwd(),"/",outdir,"/logr_R2s_mild_sham.csv", sep=""), sep=";", dec=",", row.names=TRUE, col.names=TRUE)
...
write.table(as.data.frame(aovmodels24h), paste(getwd(),"/",outdir,"/logr_models_mild_sham_24h.csv", sep=""), sep=";", dec=",")
...
write.table(Rm.total, paste(getwd(),"/",outdir,"/logr_R2s_mild_sham_24h.csv", sep=""), sep=";", dec=",", row.names=TRUE, col.names=TRUE)
