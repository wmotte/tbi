...
## Define outdir
outdir <- "/home1/michel/projects/TBI/analyses/output_11-1-2020/"
dir.create(outdir)
...
infile2 <- paste (wd, "/", study, number, "_", time, "/ROI_values/CC-", metric, "_new.txt", sep = "" )
		print(infile)
...
infile3 <- paste (wd, "/", study, number, "_", time, "/ROI_values/Hipp-", metric, "_new.txt", sep = "" )
		print(infile)
...
dir.create(paste(outdir, "combi/", sep=""))
...
# Make outdir
dir.create(paste(outdir, "behavior/", sep=""))
...
## Change labels
df.all.data$label <- paste("test")
df.all.data$label[df.all.data$Class=="Mild"] <- paste("Mild TBI")
df.all.data$label[df.all.data$Class=="Moderate"] <- paste("Moderate TBI")
df.all.data$label[df.all.data$Class=="Control"] <- paste("Sham")
df.all.data$label <- factor(df.all.data$label, levels=c("Sham", "Mild TBI", "Moderate TBI") )

## 1 decimal
scaleFUN <- function(x) sprintf("%.0f", x)

plot <- ggplot( data = df.all.data[df.all.data$variable=="cp" & df.all.data$Metric=="AD",], aes( x = Timepoint, 		y = NSS, group = label, fill = label, color = label ) ) + geom_smooth() + 					scale_y_continuous(labels=scaleFUN, breaks=scales::pretty_breaks(n=3)) +
...
plot <- ggplot( data = df.all.data[df.all.data$variable=="cp" & df.all.data$Metric=="AD",], aes( x = Timepoint, 		y = Slips, group = label, fill = label, color = label ) ) +
...
write.table(df.select, "/home1/michel/projects/TBI/analyses/data_final/diffusion_data.csv", sep=";", dec=",", row.names=FALSE)
